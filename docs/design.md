...
起動時に 320×240 の黒背景へ、白い外枠、自前の図形による顔、ASCII 文字列、96×16 の RGB565 画像を描画する。画像は赤・緑・青・白のカラーバーを big-endian の画素列としてコンパイル時に生成する。Phase 1 の実装ではフレームバッファを持たず、SPI 転送用に 512 byte のバッファだけを使った。現在の画面更新は次節のとおりフレームバッファを用いる。
...
描画は 320×240 の RGB565 のフレームバッファ (`firmware/src/framebuffer.rs`、150 KB) に行い、前回の転送から内容が変わった範囲だけを LCD へ送る。

- 以前は更新のたびに LCD 全体を黒で塗ってから描き直しており、その途中経過がちらつきとして見えた。daemon が期限を延ばすために同じ内容を送り直すたびに、画面全体がちらついていた
- フレームバッファは内部 SRAM に static に置く。動的確保は行わない。全フィールドを 0 で初期化し、`.bss` に配置して書込みイメージを増やさない。スタックは約 160 KB 残る
- 変化は転送時に検出する。描画処理は画面を黒で塗ってから描き直すため、書込み時に追跡すると元の色に戻った画素まで変化として扱ってしまう。画面を 16×16 px のタイルに分け、前回転送した内容のハッシュ (FNV-1a 64 bit) と比べ、変わったタイルをタイル行ごとに横へまとめて送る。ハッシュの保持は 2.4 KB で、LCD の内容を複製する場合 (150 KB) より少ない
- 画面全体のハッシュの計算は実機で数十 ms かかる (byte 単位の計算で約 39 ms。現在は画素単位で計算する)。前回の転送の後に書込みが無ければ計算を省く。省かない場合、メインループの 1 周がこの時間だけ延び、受信と応答が遅れる (画像領域の転送で 1 KB のフレームごとに約 85 ms を要した)
- 転送に失敗した場合は、LCD にどこまで書き込まれたかが分からないため、次回に画面全体を送る
...
本項は [プラグイン機構](plugin.md) の plugin として実装する方針で見直し中である。
