...
- 画面全体のハッシュの計算は実機で数十 ms かかる (byte 単位で約 39 ms、画素単位に変えた後も同程度の桁)。前回の転送の後に書込みが無ければ計算を省く。省かない場合、メインループの 1 周がこの時間だけ延び、受信と応答が遅れる (画像領域の転送で 1 KB のフレームごとに約 85 ms を要した)
